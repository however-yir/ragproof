...
This benchmark proves the HTTP request/response mapping, ranking metrics, citations, answerability behavior, provenance checks, and CI failure behavior are reproducible. CI also runs shuffled-ranking and missing-citation controls and asserts that both fail their intended gate. The data is explicitly synthetic; it does not claim to predict production quality or compare model vendors.
